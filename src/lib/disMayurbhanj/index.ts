...
export interface MayurbhanjDistrictData {
  district: string;
  blocks: string[];
  data: {
...
export const mayurbhanjDistrictData: MayurbhanjDistrictData = {
  district: 'Mayurbhanj',
  blocks: ['BARIPADA', 'UDALA', 'RAIRANGPUR', 'KARANJIA'],
  data: {
    gpMappings: gpMappingData,
    wasteGeneration: wasteGenerationData,
    routes: routePlanData,
  ...
  getBlockDetails: (blockName: string) => {
    ...
    const routes = routePlanData.filter(r => {
      ...
      const isBlockMatch = (targetBlock === 'baripada' && r.routeId.includes('BAR')) || 
                           (targetBlock === 'udala' && r.routeId.includes('UDA')) || 
                           (targetBlock === 'rairangpur' && r.routeId.includes('RAI')) ||
                           (targetBlock === 'karanjia' && r.routeId.includes('KARA'));
      return isStartMatch || isFinalMatch || isBlockMatch;
    });
