...
export interface KendraparaDistrictData {
  district: string;
  blocks: string[];
  data: {
...
export const kendraparaDistrictData: KendraparaDistrictData = {
  district: 'Kendrapara',
  blocks: ['Kendrapara', 'Pattamundei'],
  data: {
    gpMappings: gpMappingData,
    wasteGeneration: wasteGenerationData,
    routes: routePlanData,
  ...
  getGpDetails: (gpName: string) => {
    ...
    const schedule = collectionScheduleData.find(s => 
      s.gpName.toLowerCase().trim().includes(name) ||
      (routes.length > 0 && s.routeId === routes[0].routeId)
    );
  ...
  getBlockDetails: (blockName: string) => {
    ...
    const routes = routePlanData.filter(r => {
      ...
      const isBlockPrefixMatch = (targetBlock === 'pattamundei' && r.routeId.startsWith('KPAT'));
      const isBlockKendraMatch = (targetBlock === 'kendrapara' && r.routeId.startsWith('KKEN'));
      return isStartMatch || isFinalMatch || isBlockPrefixMatch || isBlockKendraMatch;
    });
