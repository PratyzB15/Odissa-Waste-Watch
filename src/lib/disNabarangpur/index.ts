...
export interface NabarangpurDistrictData {
  district: string;
  blocks: string[];
  data: {
...
export const nabarangpurDistrictData: NabarangpurDistrictData = {
  district: 'Nabarangpur',
  blocks: ['Nabarangpur', 'Nandahandi', 'Papadahandi', 'Kosagumuda', 'Dabugaon', 'Umerkote', 'Raighar', 'Jharigam', 'Chandahandi', 'Tentulikhunti'],
  data: {
    gpMappings: gpMappingData,
    wasteGeneration: wasteGenerationData,
    routes: routePlanData,
  ...
  getBlockDetails: (blockName: string) => {
    ...
    const routes = routePlanData.filter(r => {
      ...
      const isBlockMatch = routePrefix === blockInitial || (targetBlock === 'nabarangpur' && routePrefix === 'nab');
