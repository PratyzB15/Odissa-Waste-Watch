...
export interface CuttackDistrictData {
  district: string;
  blocks: string[];
  data: {
...
export const cuttackDistrictData: CuttackDistrictData = {
  district: 'Cuttack',
  blocks: ['ATHAGAD', 'Banki', 'Banki-Dampada', 'CUTTACKSADAR', 'TANGI CHOUDWAR'],
  data: {
    gpMappings: gpMappingData,
    wasteGeneration: wasteGenerationData,
    routes: routePlanData,
  ...
  getGpDetails: (gpName: string) => {
    ...
    const routes = routePlanData.filter(r => 
      ...
      r.intermediateGps.some(igp => igp.toLowerCase() === name) ||
      r.finalGp.toLowerCase() === name
    );
    const schedule = collectionScheduleData.find(s => 
      s.gpName.toLowerCase().includes(name) ||
  ...
  getBlockDetails: (blockName: string) => {
    ...
    const routes = routePlanData.filter(r => {
      // Resolve block by checking if any of its GPs are part of the route
