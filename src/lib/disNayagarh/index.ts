...
export interface NayagarhDistrictData {
  district: string;
  blocks: string[];
  data: {
...
export const nayagarhDistrictData: NayagarhDistrictData = {
  district: 'Nayagarh',
  blocks: ['Nayagarh', 'Daspalla', 'Ranpur', 'Khandapada', 'Odagaon'],
  data: {
    gpMappings: gpMappingData,
    wasteGeneration: wasteGenerationData,
    routes: routePlanData,
  ...
  getBlockDetails: (blockName: string) => {
    ...
    const routes = routePlanData.filter(r => {
      ...
      const isBlockMatch = routePrefix === blockInitial || (targetBlock === 'nayagarh' && routePrefix === 'nay');
