...
export interface AngulDistrictData {
  district: string;
  blocks: string[];
  data: {
...
export const angulDistrictData: AngulDistrictData = {
  district: 'Angul',
  blocks: ['Angul', 'Athamallik', 'Talcher'],
  data: {
    gpMappings: gpMappingData,
    wasteGeneration: wasteGenerationData,
    routes: routePlanData,
  ...
  getBlockDetails: (blockName: string) => {
    ...
    const routes = routePlanData.filter(r => 
      r.startingGp.toLowerCase() === blockName.toLowerCase() || // Generic block match for personnel list
