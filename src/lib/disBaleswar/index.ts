...
export interface BaleswarDistrictData {
  district: string;
  blocks: string[];
  data: {
...
export const baleswarDistrictData: BaleswarDistrictData = {
  district: 'Baleswar',
  blocks: ['Baleswar', 'Jaleswar', 'Soro', 'Nilgiri', 'Remuna'],
  data: {
    gpMappings: gpMappingData,
    wasteGeneration: wasteGenerationData,
    routes: routePlanData,
  ...
  getBlockDetails: (blockName: string) => {
    ...
    // Verified waste resolution for the block
    const waste = wasteGenerationData.filter(w => gpNames.includes(w.gpName.toLowerCase()));
    
    const routes = routePlanData.filter(r => {
      // Logic fix: Strictly partition routes by their administrative ID prefix matching the block
      // BAL -> Baleswar, JAL -> Jaleswar, SOR -> Soro, NIL -> Nilgiri, REM -> Remuna
      const prefix = r.routeId.substring(1, 4).toLowerCase();
      const blockInitial = blockName.substring(0, 3).toLowerCase();
      
      const isBlockMatch = prefix === blockInitial || (blockName.toLowerCase() === 'baleswar' && prefix === 'bal');
