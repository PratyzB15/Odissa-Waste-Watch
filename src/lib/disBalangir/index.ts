...
export interface BalangirDistrictData {
  district: string;
  blocks: string[];
  data: {
...
export const balangirDistrictData: BalangirDistrictData = {
  district: 'Balangir',
  blocks: ['Balangir', 'Gudvella', 'Patnagarh', 'Titlagarh', 'Tureikela'],
  data: {
    gpMappings: gpMappingData,
    wasteGeneration: wasteGenerationData,
    routes: routePlanData,
  ...
  getBlockDetails: (blockName: string) => {
    ...
    const routes = routePlanData.filter(r => 
      r.startingGp.toLowerCase() === blockName.toLowerCase() ||
