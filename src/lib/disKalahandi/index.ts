...
export interface KalahandiDistrictData {
  district: string;
  blocks: string[];
  data: {
...
export const kalahandiDistrictData: KalahandiDistrictData = {
  district: 'Kalahandi',
  blocks: ['Bhabanipatna', 'Dharamgarh', 'Junagarh', 'Kesinga'],
  data: {
    gpMappings: gpMappingData,
    wasteGeneration: wasteGenerationData,
    routes: routePlanData,
  ...
  getGpDetails: (gpName: string) => {
    ...
    const schedule = collectionScheduleData.find(s => s.gpName.toLowerCase().trim() === name);
  ...
  getBlockDetails: (blockName: string) => {
    ...
    const routes = routePlanData.filter(r => {
      ...
      const isBlockMatch = routePrefix === blockInitial || (targetBlock === 'bhabanipatna' && routePrefix === 'bha');
