  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (!name || !email || !password) {
      toast({ 
        ...
        description: 'Please enter your name, official email, and password to access the state dashboard.', 
        variant: 'destructive' 
      });
      return;
    }
    ...
    // Simplified open-access login simulation
    setTimeout(() => {
      toast({ 
        title: 'Welcome Back', 
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-secondary p-4">
      <Card className="w-full max-w-md shadow-lg border-2 border-primary/10">
        <CardHeader className="text-center">
          ...
          <CardDescription className="font-medium italic">Enter your administrative credentials for state-wide monitoring.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="password" className="text-xs font-black uppercase tracking-widest text-muted-foreground">Access Password</Label>
              <Input 
                id="password"
                type="password" 
                placeholder="********" 
