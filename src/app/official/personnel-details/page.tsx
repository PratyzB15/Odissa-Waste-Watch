...
import { Truck, Users, Phone, Navigation, Anchor, Search, Info, MapPin, PlusCircle, Edit, Trash2, Save } from "lucide-react";
...
function PersonnelDetailsContent() {
    ...
    useEffect(() => { 
        ...
        const activeDistrict = district || (blockName ? mrfData.find(m => m.blockCovered.toLowerCase() === blockName.toLowerCase())?.district : 'Bhadrak');
        const source = districtsSourceMap[activeDistrict?.toLowerCase() || 'bhadrak'];
        const initialRoutes = source?.getBlockDetails(blockName)?.routes || [];
        
        const enriched = initialRoutes.map((r: any) => {
            const mrfRecord = mrfData.find(m => m.blockCovered.toLowerCase() === blockName.toLowerCase());
            return {
              ...r,
              taggedMrf: mrfRecord?.mrfId || r.destination || 'District Facility'
            };
        });
        setRoutes(enriched);
    }, [blockName, district]);
    ...
    const handleDelete = (id: string | number) => {
        ...
        toast({ title: "Entry Removed", description: "Logistical circuit has been deleted from the directory." });
    };
    ...
    const handleSubmit = () => {
        ...
        if (editingRoute) {
            ...
            toast({ title: "Update Successful", description: "Route details have been synchronized." });
        } else {
            setRoutes(prev => [...prev, newRoute]);
            toast({ title: "New Entry Added", description: "Logistical circuit added to block directory." });
        }
              ...
              <CardHeader>
                <div className="flex justify-between items-start">
                    <div>
                        ...
                        <CardDescription className="text-lg">Verified collection circuits and assigned professional rosters.</CardDescription>
                    </div>
                            ...
                            <TableBody>
                                {routes.map((route, idx) => (
                                    <TableRow key={idx} className="hover:bg-primary/[0.01] transition-colors border-b last:border-0 h-24">
                                        <TableCell className="border text-[10px] font-black text-primary uppercase leading-tight">{route.taggedMrf}</TableCell>
                                        ...
                                        <TableCell className="border text-[9px] font-medium italic text-muted-foreground leading-tight">{route.intermediateGps.length > 0 ? route.intermediateGps.join(' → ') : 'Direct'}</TableCell>
                ...
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle className="text-2xl font-headline font-black uppercase text-primary flex items-center gap-2">
                            {editingRoute ? <Edit className="h-6 w-6" /> : <PlusCircle className="h-6 w-6" />}
                            {editingRoute ? 'Edit Route Entry' : 'Add New Logistical Circuit'}
                        </DialogTitle>
                        <DialogDescription>Input verified circuit data for block-wide synchronization.</DialogDescription>
                    </DialogHeader>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 py-6">
                        <div className="space-y-4">
                            <h4 className="text-[10px] font-black uppercase text-muted-foreground tracking-widest border-b pb-1">Circuit Basics</h4>
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-bold uppercase">Tagged MRF</Label>
                                <Input value={formData.taggedMrf} onChange={(e) => setFormData({...formData, taggedMrf: e.target.value})} placeholder="e.g. Kodandapur MRF" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <Label className="text-[10px] font-bold uppercase">Route ID</Label>
                                    <Input value={formData.routeId} onChange={(e) => setFormData({...formData, routeId: e.target.value})} placeholder="e.g. JJAJPCMBN" />
                                </div>
                                <div className="space-y-1.5">
                                    <Label className="text-[10px] font-bold uppercase">Abbreviation</Label>
                                    <Input value={formData.routeAbbreviation} onChange={(e) => setFormData({...formData, routeAbbreviation: e.target.value})} placeholder="e.g. PCMBN-01" />
                                </div>
                            </div>
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-bold uppercase">Total Distance (Km)</Label>
                                <Input type="number" value={formData.totalDistance} onChange={(e) => setFormData({...formData, totalDistance: e.target.value})} placeholder="0.0" />
                            </div>
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-bold uppercase">Collection Day/Schedule</Label>
                                <Input value={formData.scheduledOn} onChange={(e) => setFormData({...formData, scheduledOn: e.target.value})} placeholder="e.g. 1st & 15th of Month" />
                            </div>
                        </div>

                        <div className="space-y-4">
                            <h4 className="text-[10px] font-black uppercase text-muted-foreground tracking-widest border-b pb-1">Logistical Path</h4>
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-bold uppercase text-green-700">Starting GP</Label>
                                <Input value={formData.startingGp} onChange={(e) => setFormData({...formData, startingGp: e.target.value})} />
                            </div>
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-bold uppercase">Intermediate GPs (Comma Separated)</Label>
                                <Textarea value={formData.intermediateGps} onChange={(e) => setFormData({...formData, intermediateGps: e.target.value})} placeholder="GP1, GP2, GP3..." rows={2} />
                            </div>
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-bold uppercase text-blue-700">Last/Final GP</Label>
                                <Input value={formData.finalGp} onChange={(e) => setFormData({...formData, finalGp: e.target.value})} />
                            </div>
                            <div className="space-y-1.5">
                                <Label className="text-[10px] font-bold uppercase text-primary">Destination MRF</Label>
                                <Input value={formData.destination} onChange={(e) => setFormData({...formData, destination: e.target.value})} />
                            </div>
                        </div>

                        <div className="md:col-span-2 space-y-4">
                            <h4 className="text-[10px] font-black uppercase text-muted-foreground tracking-widest border-b pb-1">Sanitation Worker Roster</h4>
                            <Label className="text-[10px] font-bold uppercase">Format: Name:Contact (one per line)</Label>
                            <Textarea 
                                value={formData.workersText} 
                                onChange={(e) => setFormData({...formData, workersText: e.target.value})} 
                                placeholder="Ramesh Kumar:9438XXXXXX&#10;Sita Majhi:7752XXXXXX"
                                rows={4}
                                className="font-mono"
                            />
                        </div>
                    </div>
                    <DialogFooter className="bg-muted/30 p-4 -mx-6 -mb-6">
                        <Button variant="outline" onClick={() => setIsDialogOpen(false)} className="font-bold">Cancel</Button>
                        <Button onClick={handleSubmit} className="font-black uppercase px-8">
                            <Save className="mr-2 h-4 w-4" /> {editingRoute ? 'Apply Changes' : 'Save New Route'}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
...
export default function PersonnelDetailsPage() {
    return (
        <Suspense fallback={<div className="p-12 text-center text-muted-foreground animate-pulse">Syncing logistical directory...</div>}>
