...
function GPInformationContent() {
  ...
  // Get local GP data based on district and block (Fallback data)
  const localGps = useMemo((): GPRecord[] => {
    ...
    if (gps && gps.length > 0) {
        return gps.map((gp: any, idx: number): GPRecord => {
  ...
  // Sync local GP data to Firestore on first load
  const syncLocalToFirestore = useCallback(async () => {
    if (!db || !districtName || syncing || firestoreGps.length > 0) return;
    
    setSyncing(true);
    try {
      for (const localGp of localGps) {
        const documentId = `${districtName}-${localGp.gpName}`.toLowerCase().replace(/\s+/g, '-');
      ...
      toast({ 
        title: "Sync Failed", 
        description: "Failed to sync local data. Changes may not persist across devices.",
        variant: "destructive"
      });
    } finally {
      setSyncing(false);
    }
  }, [db, districtName, blockParam, localGps, firestoreGps.length, syncing, toast]);

  // Auto-sync local data to Firestore
  useEffect(() => {
    if (!loading && dataInitialized && firestoreGps.length === 0 && localGps.length > 0 && !syncing) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const documentId = editingRow?.firestoreId || (editingRow?.id?.startsWith('local-') ?
        `${districtName}-${formData.gpName}`.toLowerCase().replace(/\s+/g, '-') :
        editingRow?.id || `${districtName}-${formData.gpName}`.toLowerCase().replace(/\s+/g, '-'));
      ...
      const gpData = {
        ...
        updatedFrom: blockParam ? 'block-portal' : 'district-portal'
      };
  ...
  const handleDelete = async (row: GPRecord) => {
    ...
    const confirmDelete = confirm(`Are you sure you want to delete GP information for ${row.gpName}? This action will remove it from all portals and cannot be undone.`);
    if (!confirmDelete) return;
    
    setIsDeleting(row.gpName);
    
    try {
      if (!row.isFromFirestore) {
        const documentId = `${districtName}-${row.gpName}`.toLowerCase().replace(/\s+/g, '-');
        const gpRef = doc(db, 'gpInformation', documentId);
        ...
        <CardHeader className="bg-primary/5 border-b">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <CardTitle className="text-2xl font-bold font-headline uppercase tracking-tight">
                  Information about GPs: {districtName}
                  {blockParam && <span className="text-sm text-muted-foreground ml-2">({blockParam} Block)</span>}
                </CardTitle>
                <CardDescription>Official quantification survey data for constituent Gram Panchayats. Changes sync across all portals instantly.</CardDescription>
              </div>
            </div>
                ...
                {allGps.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={isAuthorized ? 10 : 9} className="text-center py-12 text-muted-foreground">
                      ...
                      No GP information found for {districtName}{blockParam ? ` - ${blockParam} Block` : ''}
                    </TableCell>
                  </TableRow>
                ) : (
                  allGps.map((row, idx) => (
                    <TableRow key={row.id || idx} className="hover:bg-primary/[0.02] border-b">
                      ...
                      <TableCell className="border-r font-bold uppercase leading-tight p-2">
                        <p className="text-primary text-[11px] break-words">{row.mrfName || '—'}</p>
                      ...
                      <TableCell className="text-center border-r font-mono font-bold text-[12px] p-2">{parseFloat(row.households as any).toLocaleString()}</TableCell>
                      ...
                      <TableCell className="text-right border-r font-mono font-black text-primary text-[11px] px-4 p-2">{parseFloat(row.dailyWaste as any).toLocaleString()}</TableCell>
                      <TableCell className="text-right border-r font-mono font-black text-primary text-[11px] px-4 p-2">{parseFloat(row.monthlyWaste as any).toLocaleString()}</TableCell>
                      {isAuthorized && (
                        <TableCell className="border text-center p-2">
                          <div className="flex justify-center gap-1">
                            ...
                            <Button 
                              ...
                              disabled={isDeleting === row.gpName}
                            >
                              {isDeleting === row.gpName ? (
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {editingRow 
                ...
                : 'Add new GP survey data. This will be available across district, block, and GP portals.'}
            </DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-6 py-6">
            ...
            <div className="space-y-2">
              ...
              <Input 
                ...
                placeholder="e.g., MRF Bhubaneswar"
              />
            </div>
