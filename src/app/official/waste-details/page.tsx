        ...
        <CardHeader className="bg-primary/5 border-b flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3 text-primary">
            ...
            <div>
              ...
              <CardDescription className="font-bold italic text-muted-foreground">High-fidelity oversight for {districtName}.</CardDescription>
            </div>
          </div>
        ...
        <div key={year} className="space-y-8">
          ...
          <Accordion type="single" collapsible className="w-full space-y-6">
            {MONTHS.map((month) => {
                ...
                const districtMonthAvg = mrfData.filter(m => m.district.toLowerCase() === districtName.toLowerCase()).reduce((sum, m) => sum + m.dryWasteKg, 0);
                const districtMonthVerified = monthRecords.reduce((sum, r) => sum + r.driverSubmitted, 0);
                        ...
                        <Card className="overflow-hidden border-2 shadow-xl">
                            <AccordionTrigger className="p-6 hover:no-underline bg-muted/10 data-[state=open]:bg-primary/5 transition-all border-b border-dashed">
                                <div className="flex justify-between w-full pr-8 items-center">
                                    ...
                                    <Badge variant="outline" className="font-bold border-primary/20 text-primary uppercase text-[8px] bg-primary/5 px-3">
                                        {monthRecords.length} RECEIPTS LOGGED IN DISTRICT
                                    </Badge>
                                </div>
                            </AccordionTrigger>
                            <AccordionContent className="p-6 space-y-10 bg-background">
                                <Accordion type="multiple" className="w-full space-y-4">
                                    {blockList.map((block) => {
                                        const blockMonthRecords = monthRecords.filter(r => r.block === block);
                                        
                                        return (
                                            <AccordionItem value={`${month}-${block}`} key={block} className="border rounded-xl overflow-hidden shadow-sm">
                                                <AccordionTrigger className="px-5 py-3 bg-muted/30 hover:no-underline hover:bg-primary/5 transition-all">
                                                    <div className="flex items-center gap-3">
                                                        <Building className="h-4 w-4 text-primary" />
                                                        <span className="font-bold text-xs uppercase">Block: {block}</span>
                                                    </div>
                                                </AccordionTrigger>
                                                        ...
                                                        <div className="min-w-[1600px]">
                                                            <Table className="border-collapse border text-[10px]">
                                                                ...
                                                                <TableBody>
                                                                    ...
                                                                    {blockMonthRecords.length === 0 && (
                                                                        <TableRow><TableCell colSpan={13} className="h-14 text-center italic text-muted-foreground opacity-40">No entries synced for this block in {month}.</TableCell></TableRow>
                                                                    )}
                                                                </TableBody>
                                                            </Table>
                                                        </div>
                                ...
                                {/* District Monthly Performance Summary Box Row */}
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pt-4">
                                    <div className="bg-background border-2 border-dashed rounded-xl p-5 shadow-sm">
                                        <p className="text-[10px] font-black uppercase text-muted-foreground mb-1">Dist. Load on Avg (Month)</p>
                                        <p className="text-xl font-black">{districtMonthAvg.toLocaleString()} KG</p>
                                    </div>
                                    <div className="bg-background border-2 border-dashed rounded-xl p-5 shadow-sm">
                                        <p className="text-[10px] font-black uppercase text-primary mb-1">Total District Verified</p>
                                        <p className="text-xl font-black text-primary">{districtMonthVerified.toLocaleString()} KG</p>
                                    </div>
                                    <div className="bg-background border-2 border-dashed rounded-xl p-5 shadow-sm">
                                        <p className="text-[10px] font-black uppercase text-destructive mb-1">District Discrepancy</p>
                                        <p className="text-xl font-black text-destructive">{(districtMonthAvg - districtMonthVerified).toLocaleString()} KG</p>
                                    </div>
                                    <div className="bg-primary/10 border-2 border-primary/20 rounded-xl p-5 shadow-inner">
                                        ...
                                        <p className="text-xl font-black text-primary">{(districtMonthAvg > 0 ? (districtMonthVerified / districtMonthAvg) * 100 : 0).toFixed(1)}%</p>
                                    </div>
                                </div>
                            </AccordionContent>
                        </Card>
                        ...
                        <Table className="text-muted-foreground">
                            ...
                            <TableBody>
                                {(() => {
                                  ...
                                  if (!isYearDone) {
                                    return <TableRow><TableCell colSpan={8} className="h-32 text-center italic font-black uppercase tracking-widest opacity-20">Yearly District Audit Data will populate post-December {year}.</TableCell></TableRow>;
                                  }
