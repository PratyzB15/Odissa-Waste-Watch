...
import { Calendar, Calculator, MapPin, BarChart3, Building, Warehouse } from "lucide-react";
                                    ...
                                    <Accordion type="multiple" className="w-full space-y-6">
                                        {blockGroups.map(block => (
                                            <AccordionItem value={`${year}-${month}-${block}`} key={block} className="border rounded-xl shadow-sm overflow-hidden">
                                                <AccordionTrigger className="px-6 py-4 bg-primary/5 hover:no-underline"><div className="flex items-center gap-3"><Building className="h-5 w-5 text-primary" /><h4 className="font-black uppercase text-sm text-primary">Block Node: {block}</h4></div></AccordionTrigger>
                                                    ...
                                                    <Accordion type="multiple" className="w-full space-y-4">
                                                        {Array.from(new Set(reconciledRecords.filter(r => r.block === block).map(r => r.mrf))).map(ulb => (
                                                            <AccordionItem value={`${year}-${month}-${block}-${ulb}`} key={ulb} className="border rounded-xl bg-card border-dashed">
                                                                <AccordionTrigger className="px-5 py-3 hover:no-underline bg-muted/5"><div className="flex items-center gap-2"><Warehouse className="h-4 w-4 text-muted-foreground" /><h5 className="font-bold uppercase text-[10px] text-muted-foreground">ULB Node: {ulb}</h5></div></AccordionTrigger>
                                    ...
                                    <TableBody>
                                        {(() => {
                                          ...
                                          yearly.forEach(r => {
                                              ...
                                              const prev = blockMap.get(b) || { count: 0, received: 0, paper: 0, plastic: 0, metal: 0, glass: 0, sani: 0, other: 0 };
                                              blockMap.set(b, {
                                                  count: prev.count + 1,
                                          ...
                                          return Array.from(blockMap.entries()).map(([b, stats]) => (
                                            <TableRow key={b} className="bg-primary/5 font-black text-primary h-16 hover:bg-primary/10 transition-colors">
                                                <TableCell className="border font-mono">{b}</TableCell>
