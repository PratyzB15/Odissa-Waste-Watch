                                                ...
                                                <AccordionContent className="p-6 space-y-8">
                                                    <Accordion type="multiple" className="w-full space-y-4">
                                                        {Array.from(new Set(reconciledRecords.filter(r => r.block === block).map(r => r.mrf))).map(ulb => (
                                                            <AccordionItem value={`${year}-${month}-${block}-${ulb}`} key={ulb} className="border rounded-xl bg-card">
                                                                <AccordionTrigger className="px-5 py-3 hover:no-underline"><div className="flex items-center gap-2"><Warehouse className="h-3 w-3 text-muted-foreground" /><h5 className="font-bold uppercase text-[10px] text-muted-foreground">ULB Node: {ulb}</h5></div></AccordionTrigger>
