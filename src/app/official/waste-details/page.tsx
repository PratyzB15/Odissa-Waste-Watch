...
import { Calendar, Calculator, MapPin, BarChart3, Building, Warehouse, Loader2 } from "lucide-react";
...
function DistrictWasteReconciliationContent() {
  ...
  // Get all blocks associated with this district from mrfData (pre-built cards)
  const allAssociatedBlocks = useMemo(() => {
    const blocks = new Set<string>();
    mrfData.forEach(mrf => {
      if (mrf.district.toLowerCase() === districtName.toLowerCase()) {
  ...
  // For each block, get its associated ULBs (pre-built cards)
  const blockToUlbsMap = useMemo(() => {
    const map = new Map<string, string[]>();
    allAssociatedBlocks.forEach(block => {
      const ulbs = new Set<string>();
      mrfData.forEach(mrf => {
        if (mrf.district.toLowerCase() === districtName.toLowerCase() && 
            mrf.blockCovered.toLowerCase() === block.toLowerCase()) {
          ulbs.add(mrf.ulbName);
        }
      });
      map.set(block, Array.from(ulbs).sort());
    });
    return map;
  }, [allAssociatedBlocks, districtName]);
  ...
  // Calculate district baseline average from GP Information page
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">Loading district waste reconciliation data...</p>
        </div>
      </div>
    );
  }
        ...
        <CardHeader className="bg-primary/5 border-b">
          <div className="flex items-center gap-3 text-primary">
            ...
            <div>
              ...
              <CardDescription className="font-bold italic text-muted-foreground">Authoritative district oversight for {districtName}.</CardDescription>
            </div>
          </div>
        </CardHeader>
      </Card>
        ...
        <div key={year} className="space-y-8">
          ...
          <Accordion type="single" collapsible className="w-full space-y-8">
            {MONTHS.map((month) => {
              ...
              // Calculate district totals for the month
              ...
              // Group records by Block, then by ULB
              const blockRecordsMap = new Map<string, WasteRecord[]>();
              const blockUlbsMap = new Map<string, Map<string, WasteRecord[]>>();
              
              // Initialize all blocks with empty arrays
              allAssociatedBlocks.forEach(block => {
                blockRecordsMap.set(block, []);
                const ulbMap = new Map<string, WasteRecord[]>();
                const ulbsForBlock = blockToUlbsMap.get(block) || [];
                ulbsForBlock.forEach(ulb => {
                  ulbMap.set(ulb, []);
                });
                blockUlbsMap.set(block, ulbMap);
              });
              
              // Populate with actual records
              monthItems.forEach(record => {
                if (record.block && blockRecordsMap.has(record.block)) {
                  const existing = blockRecordsMap.get(record.block) || [];
                  existing.push(record);
                  blockRecordsMap.set(record.block, existing);
                  
                  if (record.ulb) {
                    const ulbMap = blockUlbsMap.get(record.block);
                    if (ulbMap && ulbMap.has(record.ulb)) {
                      const ulbExisting = ulbMap.get(record.ulb) || [];
                      ulbExisting.push(record);
                      ulbMap.set(record.ulb, ulbExisting);
                    }
                  }
                }
              });
                  ...
                  <Card className="overflow-hidden border-2 shadow-xl">
                    ...
                    <AccordionContent className="p-6 space-y-8 bg-background">
                      ...
                      <Accordion type="multiple" className="w-full space-y-6">
                        {allAssociatedBlocks.map((block) => {
                          const blockRecords = blockRecordsMap.get(block) || [];
                          const blockTotals = calculateTotals(blockRecords);
                          const ulbMap = blockUlbsMap.get(block) || new Map();
                          
                          return (
                            <AccordionItem value={`${year}-${month}-${block}`} key={block} className="border rounded-xl shadow-sm overflow-hidden">
                              <AccordionTrigger className="px-6 py-4 bg-primary/5 hover:no-underline">
                                <div className="flex items-center gap-3">
                                  <Building className="h-5 w-5 text-primary" />
                                  <h4 className="font-black uppercase text-sm text-primary">Block Node: {block}</h4>
                                </div>
                              </AccordionTrigger>
                              <AccordionContent className="p-6 space-y-6">
                                
                                {/* ULB-wise Cards inside Block */}
                                <Accordion type="multiple" className="w-full space-y-4">
                                  {Array.from(ulbMap.entries()).map(([ulb, ulbRecords]) => {
                                    const ulbTotals = calculateTotals(ulbRecords);
                                    
                                    return (
                                      <AccordionItem value={`${year}-${month}-${block}-${ulb}`} key={ulb} className="border rounded-xl bg-card border-dashed">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 p-6 bg-muted/5 border-t mt-6 rounded-xl">
                                  <div className="bg-background border-2 border-dashed rounded-xl p-5 shadow-sm">
                                    ...
                                    <p className="text-2xl font-black">{blockTotals.totalFromGPs.toLocaleString()} KG</p>
                                  </div>
                                  <div className="bg-background border-2 border-dashed rounded-xl p-5 shadow-sm">
                                    <p className="text-[10px] font-black uppercase text-primary mb-1">Block Verified</p>
                                    <p className="text-2xl font-black text-primary">{blockTotals.driverSubmitted.toLocaleString()} KG</p>
                                  </div>
                                  <div className="bg-background border-2 border-dashed rounded-xl p-5 shadow-sm">
                                    <p className="text-[10px] font-black uppercase text-destructive mb-1">Block Discrepancy</p>
                                    <p className="text-2xl font-black text-destructive">{blockTotals.discrepancy.toLocaleString()} KG</p>
                                  </div>
                                  <div className="bg-primary text-primary-foreground rounded-xl p-5 shadow-lg">
                                    <p className="text-[10px] font-black uppercase opacity-60 mb-1">Block Efficiency</p>
                                    <p className="text-3xl font-black">{blockTotals.driverSubmitted > 0 ? ((blockTotals.driverSubmitted / (blockTotals.totalFromGPs || 1)) * 100).toFixed(1) : 0}%</p>
                                  </div>
                                </div>
                              </AccordionContent>
                            </AccordionItem>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 p-6 bg-muted/5 border-t mt-8 rounded-xl">
                        <div className="bg-background border-2 border-dashed rounded-xl p-5 shadow-sm">
                          ...
                          <p className="text-2xl font-black">{districtBaselineAvg.toLocaleString()} KG</p>
                        </div>
                        <div className="bg-background border-2 border-dashed rounded-xl p-5 shadow-sm">
          ...
          <Card className="mt-12 border-4 border-dashed border-primary/30 bg-muted/5 overflow-hidden">
            <CardHeader className="bg-primary/5 border-b border-dashed border-primary/20 pb-8 text-center">
              <CardTitle className="text-4xl font-black font-headline uppercase tracking-tight text-primary/40 flex items-center justify-center gap-4">
                <BarChart3 className="h-12 w-12" /> Yearly District Audit: {year}
              </CardTitle>
            </CardHeader>
                ...
                <div className="min-w-[1600px]">
                  <Table>
                    <TableHeader className="bg-muted/50">
                      <TableRow>
                        <TableHead className="w-[200px] uppercase font-black border">Block Node</TableHead>
                    ...
                    <TableBody>
                      {(() => {
                        ...
                        yearly.forEach(r => {
                          ...
                          const prev = blockMap.get(block) || { count: 0, received: 0, plastic: 0, paper: 0, metal: 0, glass: 0, sani: 0, other: 0 };
                          blockMap.set(block, {
                            count: prev.count + 1,
                        ...
                        return Array.from(blockMap.entries()).map(([block, stats]) => (
                          <TableRow key={block} className="bg-primary/5 font-black text-primary h-16 hover:bg-primary/10 transition-colors">
                            <TableCell className="border font-mono">{block}</TableCell>
