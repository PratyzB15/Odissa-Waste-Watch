        ...
        <div key={year} className="space-y-8">
            ...
            <Accordion type="single" collapsible className="w-full space-y-6">
                {MONTHS.map((month, mIdx) => {
                    ...
                    }), { plastic: 0, paper: 0, metal: 0, glass: 0, sanitation: 0 });
                                    ...
                                    <Accordion type="multiple" className="w-full space-y-6">
                                        {districtGroups.map(dist => (
                                            <AccordionItem value={`${year}-${month}-${dist}`} key={dist} className="border rounded-xl shadow-sm overflow-hidden">
                                                <AccordionTrigger className="px-6 py-4 bg-primary/5 hover:no-underline"><div className="flex items-center gap-3"><DistrictIcon className="h-5 w-5 text-primary" /><h4 className="font-black uppercase text-lg text-primary">District: {dist}</h4></div></AccordionTrigger>
                                                <AccordionContent className="p-6 space-y-8">
                                                    <Accordion type="multiple" className="w-full space-y-4">
                                                        {Array.from(new Set(reconciledRecords.filter(r => r.district === dist).map(r => r.block))).sort().map(block => (
                                                            <AccordionItem value={`${year}-${month}-${dist}-${block}`} key={block} className="border rounded-xl bg-card">
                                                                <AccordionTrigger className="px-5 py-3 hover:no-underline"><div className="flex items-center gap-2"><Building className="h-4 w-4 text-muted-foreground" /><h5 className="font-bold uppercase text-xs text-muted-foreground">Block: {block}</h5></div></AccordionTrigger>
                                                                <AccordionContent className="p-4 space-y-4">
                                                                    {Array.from(new Set(reconciledRecords.filter(r => r.district === dist && r.block === block).map(r => r.mrf))).sort().map(ulb => (
                                                                        <div key={ulb} className="space-y-2">
